    ...
    <div class="w-full max-w-sm bg-zinc-950 rounded-lg shadow-2xl border border-zinc-800 p-5">
      ...
      <div class="flex items-center gap-3 mb-4">
        <div class="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center shrink-0">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-zinc-300"
        ...
        <div>
          <h3 class="text-white text-sm font-semibold">Update Available</h3>
          <p class="text-zinc-500 text-xs">Version {{ version }}</p>
        </div>
      </div>
      ...
      <template v-else-if="stage === 'downloading'">
        <div class="flex items-center gap-3 py-2">
          <svg class="animate-spin w-4 h-4 text-zinc-400 shrink-0"
               xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10"
                    stroke="currentColor" stroke-width="4" />
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
          </svg>
          <span class="text-xs text-zinc-400">Downloading installer…</span>
        </div>
      </template>

      <!-- Stage: downloaded -->
      <template v-else-if="stage === 'downloaded'">
        <p class="text-xs text-zinc-400 mb-1">Download complete.</p>
        <p class="text-xs text-zinc-600 mb-4 truncate" :title="downloadedPath">
          {{ downloadedPath }}
        </p>
        <div class="flex justify-end gap-2">
          <button @click="dismiss"
      ...
      <!-- Controller hint -->
      <p class="mt-3 text-xs text-zinc-600 text-right" v-if="stage !== 'downloading'">
        <kbd class="px-1 py-0.5 rounded bg-zinc-800 text-zinc-400 font-mono">A</kbd> confirm
        &nbsp;·&nbsp;
        <kbd class="px-1 py-0.5 rounded bg-zinc-800 text-zinc-400 font-mono">B / Esc</kbd> dismiss
      </p>
    </div>
  </div>
</template>
